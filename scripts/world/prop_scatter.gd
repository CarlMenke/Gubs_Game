...
const DENSE_LAYERS := [
	{
		...
		"cell": 4.2, "chance": 0.55, "scale": [0.45, 0.85], "align": 0.25,
	...
	},
	{
		...
		"cell": 3.0, "chance": 0.55, "scale": [0.22, 0.42], "align": 0.35,
	...
	},
	{
		...
		"models": ["Grass_Common_Short", "Grass_Common_Tall", "Grass_Wispy_Short",
			"Grass_Wispy_Tall"],
		"cell": 1.30, "chance": 0.85, "scale": [0.5, 1.0], "align": 0.4,
		"slope_max": 0.95, "rim_margin": 0.3, "sink": 0.06,
		"radial_power": 1.0, "clump": 0.35, "clump_frequency": 0.09, "shadows": false,
	},
	{
		"name": "Clover",
		"models": ["Clover_1", "Clover_2"],
		"cell": 2.6, "chance": 0.7, "scale": [0.4, 0.75], "align": 0.5,
	...
	},
	{
		...
		"cell": 3.1, "chance": 0.65, "scale": [0.32, 0.6], "align": 0.2,
	...
	},
	{
		...
		"cell": 4.6, "chance": 0.55, "scale": [0.7, 1.5], "align": 0.5,
	...
	},
	{
		...
		"cell": 3.0, "chance": 0.6, "scale": [0.5, 1.3], "align": 0.9,
	...
	},
	{
		...
		"cell": 3.4, "chance": 0.6, "scale": [0.5, 1.0], "align": 1.0,
...
## layer whose count quietly depends on how lucky the seed was.
const SPARSE_ATTEMPTS := 40
...
func _scatter_sparse(parent: Node3D, layer: Dictionary) -> void:
	...
	while placed < wanted and attempts > 0:
		...
		_occupied.append(Vector3(spot.x, spot.y, float(layer["reserve"]) * uniform))
		if layer["name"] == "Trees":
...
func _candidate(layer: Dictionary) -> Vector2:
	var reach := _island.extent()
	var spot := Vector2(_rng.randf_range(-reach, reach), _rng.randf_range(-reach, reach))
	return spot if _usable(spot, layer) else Vector2.INF


func _usable(spot: Vector2, layer: Dictionary) -> bool:
	var mass := _island.landmass_at(spot.x, spot.y)
	if mass == null:
		return false
	...
	if _island.inset_at(spot.x, spot.y) < float(layer["rim_margin"]):
		return false
...
func _too_close(spot: Vector2, gap: float) -> bool:
	for entry: Vector3 in _occupied:
		# Dense layers pass gap 0 and are only kept out of the prop's own
		# footprint; sparse layers add their own spacing on top of it.
		var limit := maxf(gap, entry.z * 0.45)
